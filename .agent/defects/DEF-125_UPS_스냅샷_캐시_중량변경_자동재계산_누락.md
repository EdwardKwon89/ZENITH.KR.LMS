# DEF-125: UPS 스냅샷 캐시 — 중량/부피 변경 시 자동 재계산 메커니즘 누락
...
> **2026-07-24 재채번 이력**: DEF-120(최초) → DEF-121(Team B TASK-B-192/Issue #728과 충돌) → DEF-124(그 사이 Team B가 먼저 등록한 또 다른 DEF-121, Issue #741/TASK-B-193과 재충돌 발견) → **DEF-125**(TeamB_Dev 병합 후 Baker의 TASK-B-197/Issue #778이 이미 마이그레이션 파일명(`20260724000000_def124_...sql`)에 DEF-124를 쓰고 있던 것을 뒤늦게 발견, 3차 재정정). Baker 쪽은 이미 적용된 DB 마이그레이션 파일명이라 정정하지 않고 Team A 쪽을 계속 조정(Edward 결정, 2026-07-24).
...
**잔여 갭**: 재계산 시 중량은 전 패키지 합산(정상)이지만 **치수(L×W×H)는 첫 번째 패키지만 사용** — 멀티패키지 오더에서 2번째 이후 패키지의 치수 변경이 반영되지 않음. 이 갭은 입고재계산뿐 아니라 오더 등록 시점(`saveOrderRateSnapshot`)에도 동일하게 존재하는, 더 오래된(2026-07-05부터) 근본 갭이다. 별도 결함으로 등록: **[DEF-132](DEF-132_운임스냅샷_멀티패키지_치수_첫번째만_반영.md)**.

> **2026-07-29 채번 정정**: 최초 "DEF-B-021"로 등록했으나 Team B가 같은 번호를 이미 사용 중(Issue #945·TASK-B-242, 무관한 별개 결함)이던 것을 뒤늦게 발견 — Aiden(Team A) 발견 건이므로 애초에 Team A 접두사(`DEF-NNN`)를 썼어야 했다. `DEF-132`로 재채번.
