...
| Baker | 채번 절차 누락 (TASK-B-103 중복, Dave의 기병합 Issue #340과 동일 번호) | **1회** | Issue #429 (PR#431) | 정기 점검 중 Aiden 발견. TASK-B-103은 이미 Dave가 Issue #340에서 사용해 병합 완료(PR#368, `.agent/ACTIVE_TASK.md` 356행)된 번호. PR#431 병합 보류, Jaison에게 재채번 요청. **같은 유형(채번 절차 누락)이 최근 Dave(2회: TASK-B-105, TASK-B-114)에 이어 Baker에서도 발생 — 개별 Agent 문제가 아니라 Team B 착수 절차에서 `next-task-number.sh` 사용이 구조적으로 누락되고 있을 가능성**. Jaison에게 절차적 보완(체크박스·훅 등) 검토 요청. 할당 중단 기준(3회) 미달 |
...
_최종 갱신: 2026-07-14 (Baker TASK-B-119 상태전환 누락 2회째 — PR#452 반려·재작업 요청 / Dave 5회 건 할당 중단 여부 최종 판단 여전히 대기)_
