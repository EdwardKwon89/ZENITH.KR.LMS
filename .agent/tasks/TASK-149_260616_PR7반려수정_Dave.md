# TASK-149 — PR#7 반려 수정 (Dave 담당) — Issue 1·5·7
...
```
1. 코드 커밋: [Dave] fix: TASK-149 PR#7 반려 수정 — _linkShipperToAgency 컬럼·i18n·TC (Issue 1·5·7)
...
6. 문서 커밋: [Dave] docs: TASK-149 완료 보고 — PR#7 반려 수정 🔔
```
...
- [ ] `shippers.ts` `_linkShipperToAgency` — `...data` spread 제거, 명시적 5개 컬럼 지정
- [ ] `agency/page.tsx` — 하드코딩 한글 2곳 `t()` 키 교체
- [ ] `messages/ko.json` + `messages/en.json` — `agency_console_badge`, `agency_console_desc` 추가
- [ ] `shipper-actions.test.ts` — TC 제목 불일치 수정
