...
- [ ] UAT_DEFECT_LOG.md — DEF-032 상태 수정완료·커밋 해시 기재
- [ ] UAT_DEFECT_LOG.md — DEF-035 상태 수정완료·커밋 해시 기재
- [ ] UAT_DEFECT_LOG.md — 현황 요약 기능오류 미수정 수치 갱신
- [ ] UAT_10 — Rate Card 폼 검증 항목에 carrier_cost / margin_rate / platform_fee_rate 3개 필드 추가
- [ ] TISA Dashboard 시나리오 A (Admin 전체 필드) 추가
- [ ] TISA Dashboard 시나리오 B (Shipper BaseAmount+Currency 만) 추가
- [ ] TISA Dashboard 시나리오 C (경로 미선택 fallback) 추가
- [ ] UAT_MASTER.md 합계 갱신
- [ ] 회귀 테스트 전체 PASS (문서 변경이므로 테스트 영향 없으나 의무 실행)
- [ ] 코드 커밋 완료 (커밋 해시 기재)
- [ ] task file `[작업 결과]` 섹션 기재 + 상태 🔔로 변경
- [ ] ACTIVE_TASK.md 상태 🔄→🔔 반영
- [ ] `scratch/IMP_PROGRESS.md` 해당 행 갱신 불필요 (문서 보완 전용)
