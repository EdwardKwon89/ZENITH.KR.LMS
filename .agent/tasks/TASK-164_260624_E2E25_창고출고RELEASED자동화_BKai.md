...
- [ ] UAT-18-01 (RELEASED 전이) Playwright 전환 PASS — **블로커**: PR #103 머지 선행 필요
- [ ] UAT-18-02 (intl_ref_no 연동) Playwright 전환 PASS — **블로커**: PR #103 머지 선행 필요
- [x] seed 데이터 보강 완료 (창고 입고 fixture)
- [ ] 로컬 실행 전 케이스 PASS — **블로커**: PR #103 머지 후 가능
- [x] 회귀 테스트 전체 PASS (`vitest run`) — 387/387
- [x] B_Kai 코드: packages 컬럼 제거 + OutboundProcessForm.tsx order.order_packages 전환 (`b71a90b`)
...
| 항목 | 파일 | 커밋 |
|:----|:----|:----:|
| `getWarehousedOrders()` SELECT에서 `packages,` 제거 | `warehouse.ts:27` | `b71a90b` |
| `order_packages` FK join에 `gross_weight` 추가 | `warehouse.ts:31` | `b71a90b` |
| `getTodayReleasedOrders()` SELECT에서 `packages` 제거 | `warehouse.ts:61` | `b71a90b` |
| `order.packages` → `order.order_packages` (2곳) | `OutboundProcessForm.tsx:167,255` | `b71a90b` |

**상태**: 🔴 D_Kai PR #103 (`feature/teama-issue-95-getWarehousedOrders-fix` → `develop`) 머지 대기 중. 머지 후 rebase → E2E 실행 → PR 생성.
