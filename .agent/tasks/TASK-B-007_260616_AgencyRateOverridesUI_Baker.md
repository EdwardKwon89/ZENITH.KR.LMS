...
- [ ] `NaviSidebar.tsx` line 146 직후 `agency_rate_overrides_nav` 메뉴 추가 확인
- [ ] `messages/ko.json` — 10개 i18n 키 추가 완료
- [ ] `messages/en.json` — 10개 i18n 키 추가 완료
- [ ] `/agency/rate-overrides/page.tsx` (서버) — 목록 페이지 구현 완료
- [ ] `/agency/rate-overrides/new/page.tsx` (서버) — 등록 페이지 구현 완료
- [ ] 클라이언트 컴포넌트 5종 — 각 50줄 이하 (ZEN_A4) 확인
- [ ] `deactivateAgencyRateOverride` 호출 + `router.refresh()` 동작 확인
- [ ] `upsertAgencyRateOverride` 폼 제출 + redirect 동작 확인
- [ ] `npm run test:regression` 전체 PASS
- [ ] TS 빌드 PASS (`tsc --noEmit` 오류 없음)
- [ ] 코드 커밋 해시: `TBD`
- [ ] 문서 커밋 해시: `TBD`
