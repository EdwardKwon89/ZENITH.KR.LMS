...
**제출**: 🔔 재작업 완료 (2026-06-15)

**반려 사항 해소 상태**:
1. ✅ **커밋 해시 정정**: `b7e1f2a` → `b315d49` (코드) + `6870271` (테스트 수정)
2. ✅ **회귀 테스트 전체 실행**: `npm run test:regression` — 351/357 PASS (6건 실패는 기존 `p6-transport-policy.test.ts` — TASK-144 무관)
3. ✅ **LIVE_REGRESSION_TEST_MAP.md 등재**: TC-WH-REF-01~04 4건 추가 (총 353→357 Cases)

**검토 요청 사항**:
- TASK-144 코드 리뷰 및 승인
- `feature/ups-spr02-bkai-warehouse-ref` → `feature/ups-spr02-aiden-pricing-engine` 통합 머지 타이밍 확인 (TASK-141 완료 후)

**참고**: `src/components/warehouse/InboundProcessForm.tsx`의 `handleSavePkgRefs`는 `getOrderByBarcodeOrNo`를 재조회하여 UI 동기화 — 이 패턴이 적절한지 확인 필요
