...
`/tmp/review-pr1086-v2` 격리 워크트리에서 fresh `supabase db reset`(신규 마이그레이션 2건 정상 적용) 후 재검증.

**v1 반려** — `translateShxkText()` 완전일치 조회가 이번 Task 근거가 된 실측 응답(`1ZJ443D30403088388`)의 실제 문장과 불일치해 번역이 항상 실패함을 vitest로 실제 함수 직접 호출해 확인(`null` 반환 재현), PR 코멘트로 반려 및 Issue `status:rework` 전환.

**v2 재검증** — 부분일치 매칭으로 수정 확인. 동일한 방식으로 실측 두 문장을 직접 재호출해 정상 번역 확인. **독립 되돌리기 검증**: `translateShxkText()`를 완전일치 버전으로 수동 되돌려 신규 3건이 정확히 FAIL(`expected null not to be null` 등) 재현 → 복원 후 15/15 재확인. `storeTrackingEvents()` dedup 로직도 별도로 구버전(`event_time`만 비교)으로 되돌려 TC-290-01 FAIL(`expected length 1 but got 2`) 재현 → 복원 후 7/7 재확인. 전체 회귀 **179/179·1268/1268 ALL PASS**(재검증 일치) · `npm run build` SUCCESS · 실제 CI(`gh pr checks 1086`) 3개 항목 전부 pass. PR#1086 승인·머지(TeamB_Dev `61a2fd07`), Issue #1085 종결.
