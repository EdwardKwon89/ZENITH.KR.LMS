...
| 검토일 | 2026-05-20 |
| 판정 | ❌ 반려 |
| 검토 의견 | 6종 보안 헤더 구현 확인 (CSP·HSTS·X-Frame-Options·X-Content-Type-Options·Referrer-Policy·Permissions-Policy) ✅. `unsafe-inline`/`unsafe-eval` 포함은 Next.js 호환성상 불가피하므로 허용 — 추후 nonce-based CSP로 강화 권고(별도 IMP). **DoD 미달성으로 반려**: ① 회귀 결과 파일 `docs/08_Self_Audit/Regression_Results/` 미저장 (R-13 위반) ② DoD 체크리스트 전항목 `[ ]` 미체크. **재작업 요구**: ① 회귀 테스트 재실행 + 결과 파일 저장 ② DoD 체크리스트 `[x]` 완료. (gitnexus_impact — next.config.ts는 코드 심볼 아님, 면제). |
