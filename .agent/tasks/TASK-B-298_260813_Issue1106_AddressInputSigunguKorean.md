...
| **생성일** | 2026-08-13 |
| **우선순위** | P2 |
| **상태** | ✅ 완료 (단, 실브라우저 라이브 검증은 JSJung 대기 중) |
...
`/tmp/review-pr1107` 격리 워크트리에서 재검증. 매핑 테이블 12개 항목 전부 제가 착수 승인 시 라이브러리 데이터와 직접 대조한 값과 정확히 일치. `mappedCityName ?? matched?.name ?? sigunguEn` 우선순위(한글 매핑 → DEF-B-063 폴백 → 원본값)도 설계 그대로. 신규 테스트 6건 전부 `AddressInput` mock 없이 실제 컴포넌트 렌더링 — 그림자 패턴 없음. 6/6 PASS.

**독립 되돌리기 검증**: `mappedCityName ??` 우선순위를 제거 → **TC-298-01~05 정확히 FAIL**(빈 문자열 mock 조건에서 폴백도 실패하는 것을 실증), TC-298-06(서울, 폴백 경로)은 영향 없이 PASS 유지 → 복원 후 6/6 재확인.

전체 회귀 **191/191·1315/1315 ALL PASS**(재검증 일치) · `npm run build` SUCCESS · 실제 CI(`gh pr checks 1107`) 3개 항목 전부 pass. PR#1107 승인·머지(TeamB_Dev `43fac962`), Issue #1106 종결.

**R-10 관련 유보 사항**: task file에 "이번엔 생략 불가"로 명시했으나 Dave도 Jaison도 실브라우저 환경이 없어 Kakao의 실제 응답을 직접 확인할 수 없었음. 이번 수정은 "`sigunguEnglish`가 비어있을 것"이라는 가설에 기반 — 코드 자체는 견고하나(한글 필드는 API 핵심 응답이라 신뢰 가능) 대왕판교로의 실제 응답이 이 가설과 정확히 일치하는지는 아직 실증되지 않음. PR 코멘트·Issue 종결 코멘트에 명시해 JSJung의 실사용 확인을 요청함.
