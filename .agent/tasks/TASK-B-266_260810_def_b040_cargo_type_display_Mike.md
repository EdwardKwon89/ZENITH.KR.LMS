# TASK-B-266 — DEF-B-040 Agency/Shipper UPS 요율조회 화면 cargo_type 축 반영

| 항목 | 내용 |
|:---|:---|
| Task-ID | TASK-B-266 |
| Issue-ID | #1027 / DEF-B-040 |
| 생성일 | 2026-08-10 |
| 담당 Agent | Mike (MiMo V2.5) |
| 우선순위 | P1 (defect:high) |
| 상태 | ✅ 완료 |

---

## 배경

JSJung이 "master air" 계정으로 `/agency/ups-rates` 확인 중, 등록한 할인율(DOC 55%/NON_DOC 75%)이 "대리점 원가" 표시에 이상하게 적용되는 것을 발견. Issue #1018(cargo_type 축 도입) 당시 조회 화면이 갱신되지 않아 Zone당 할인율 1개로 가정 중.

---

## 변경 파일

| 파일 | 변경 내용 |
|:-----|:----------|
| `src/lib/ups/cargo-type-utils.ts` | 신규: candidateCargoTypes/resolveDiscountRate 분리 |
| `src/app/actions/ups/rates-public.ts` | product join에 cargo_type 추가 |
| `agency-ups-rates-client.tsx` | policyByZone 중첩 구조 + import |
| `UpsBaseRateMatrix.tsx` | discountRateMap 중첩 구조 + import |
| `shipper/ups-rates/page.tsx` | zoneDiscountMap 중첩 구조 |
| `shipper/ups-rates-client.tsx` | import |
| `def-b040-cargo-type-display-component.test.tsx` | 회귀 테스트 9개 (React Testing Library) |

---

## 핵심 로직 (freight.ts와 동일 패턴)

```typescript
function candidateCargoTypes(productCargoType?: string): string[] {
  if (productCargoType === 'DOC') return ['DOC', 'ALL'];
  if (productCargoType === 'NON_DOC') return ['NON_DOC', 'ALL'];
  // BOTH(Expedited/Flight): NON_DOC 우선, ALL 폴백
  return ['NON_DOC', 'ALL'];
}
```

---

## [작업 결과]

**커밋**: `27553f3f` — `[Mike] fix: DEF-B-040 Agency/Shipper UPS 요율조회 화면 cargo_type 축 반영 (Issue #1027)`

**PR**: #1033 (TeamB_Dev base) — https://github.com/EdwardKwon89/ZENITH.KR.LMS/pull/1033

**변경 파일 7개**:
1. `src/lib/ups/cargo-type-utils.ts` (신규)
2. `src/app/actions/ups/rates-public.ts`
3. `src/app/[locale]/(dashboard)/agency/ups-rates/agency-ups-rates-client.tsx`
4. `src/components/ups/UpsBaseRateMatrix.tsx`
5. `src/app/[locale]/(dashboard)/shipper/ups-rates/page.tsx`
6. `src/app/[locale]/(dashboard)/shipper/ups-rates/shipper-ups-rates-client.tsx`
7. `tests/unit/ups/def-b040-cargo-type-display-component.test.tsx` (신규)

**회귀 테스트 9개 (React Testing Library 실제 컴포넌트 렌더링)**:
- resolveDiscountRate 실제 함수 검증 (4건)
- candidateCargoTypes 실제 함수 검증 (1건)
- AgencyUpsRatesClient 컴포넌트 렌더링 (3건)
- 되돌리기 검증 (1건)

**핵심 테스트 46개 전부 통과**

---

## 검증 결과

- TypeScript 타입 체크: ✅ 통과
- 회귀 테스트: ✅ 9개 (실제 컴포넌트 렌더링)
- 핵심 단위 테스트: ✅ 46개 전부 통과
