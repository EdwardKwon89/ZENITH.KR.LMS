...
- [x] `imp049` 수정 REVERT + 신규 `20260522000300_fix_preferred_language_view.sql` migration 생성
- [x] `scratch/20260522000100_fix_zen_orders_updated_at.sql` → `supabase/migrations/` 복원 (Option A 확정)
- [x] `20260522000200_restore_zen_organizations_columns.sql` 포함 커밋
- [x] `supabase db push --local` 정상 완료 확인
- [x] spec 타이밍 최적화: `waitForTimeout(3000)×4` 제거 — `not.toBeVisible({timeout:10000})` 대체 적용 (TASK-057 통합)
- [x] 케이스 A (RETURNED→WAREHOUSED) E2E PASS — 스크린샷 3장 (`e2e_14_a_01~03.png`)
- [x] 케이스 B (RETURNED→DISPOSED) E2E PASS — 스크린샷 2장 (`e2e_14_b_01~02.png`)
- [x] `docs/99_Manual/E2E_14_Result/RESULT.md` 케이스 A/B PASS 갱신
- [x] 회귀 테스트 211/211 PASS (`REGRESSION_2026-05-22_TASK-053-v2.log`)
- [x] `[B_Kai] test: E2E-14 재작업` 코드 커밋 완료 (`e70b6a2`)
- [x] `[B_Kai] docs: TASK-053 재작업` 문서 커밋 완료 (`51cd330`)
...
| # | 2차 DoD 항목 | 판정 | 비고 |
|:-:|:---------|:----:|:-----|
| 1 | `imp049` REVERT + `000300` migration 생성 | ✅ | `git checkout` 확인 + `000300` 파일 정상 |
| 2 | `000100` updated_at migration 복원 | ✅ | `e70b6a2` 포함 확인 |
| 3 | `000200` organizations migration 포함 커밋 | ✅ | `e70b6a2` 포함 확인 |
| 4 | `supabase db push --local` PASS | ✅ | B_Kai 기재 (Step 4) |
| 5 | spec 타이밍 최적화 (`waitForTimeout×4` 제거) | ✅ | `not.toBeVisible()` 방식 적용 — 기능 동등 |
| 6 | 케이스 A E2E PASS + 스크린샷 | ✅ | 3장 확인 (`e2e_14_a_01~03.png`) |
| 7 | 케이스 B E2E PASS + 스크린샷 | ✅ | 2장 확인 (`e2e_14_b_01~02.png`) |
| 8 | RESULT.md 갱신 | ✅ | `e70b6a2` 포함 확인 |
| 9 | 회귀 211/211 PASS | ✅ | `REGRESSION_2026-05-22_TASK-053-v2.log` |
| 10 | 코드 커밋 완료 | ✅ | `e70b6a2` |
| 11 | 문서 커밋 완료 | ✅ | `51cd330` |
| 12 | 상태 🔔 + ACTIVE_TASK 동기화 | ✅ | 확인 |

### 최종 판정: ✅ PASS

12단계 재작업 전량 완료. migration 3종 적용 + E2E 케이스 A/B PASS + 회귀 211/211 전량 확인. IMP-060 완료.

### Advisory

- **DoD 체크박스 미체크**: 3차 재작업 완료 후 2차 DoD 체크박스를 `[x]`로 업데이트하지 않음. Aiden 직접 보완. 차회 작업 시 DoD 체크박스 업데이트 필수.
- **타이밍 최적화 방식**: 지시(`span:has-text("RETURNED")` → `toBeVisible`)와 다른 방식(`h3:has-text("상태 변경")` → `not.toBeVisible`) 적용. 기능적으로는 모달 닫힘 확인 후 reload로 상태 검증하는 올바른 패턴 사용. 결과적으로 적합하나 지시 준수 정확도 주의.
- **개정이력 형식 오류**: B_Kai가 기존 Aiden 행을 덮어써 형식이 깨짐. Aiden 직접 정정.
...
| 2026-05-22 | B_Kai | 3차 재작업 완료 — 12단계 전량 이행. 코드 e70b6a2 · 문서 51cd330 · E2E A/B ✅ · 회귀 211/211 · 🔔 |
| 2026-05-22 | Aiden (Claude) | TASK-053 ✅ PASS — 3차 재작업 전량 확인. DoD 2차 전량 이행. IMP-060 완료. Aiden 직접 보완: 2차 DoD 체크박스·개정이력 형식 정정 |
