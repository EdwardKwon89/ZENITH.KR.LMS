# TASK-203: Issue #736 — W2: 최종 운임 확정 UI(UPS 발송 기준) + 화주별 일별 청구 집계 구현 (Riley)
...
1. **서버 액션 (`src/app/actions/finance/daily-billing.ts`)**:
   - `getShipperDailyBillingSummary({ startDate, endDate, shipperId })`: 화주별×일자별(`YYYY-MM-DD`) 오더수, 기본운임, 유류할증료, 급증수수료, 사후조정액, 총 합계액(USD 및 환율 반영 KRW), 마감/미마감 건수 집계.
...
- [x] 단위 테스트 작성 (`tests/unit/finance/daily-billing-aggregation.test.ts` 4/4 PASS)
...
1. **[코드 커밋]** `[Gemini] feat: TASK-203 W2 — 최종 운임 확정 UI(UPS 발송 기준) + 화주별 일별 청구 집계 — Issue #736 구현`
2. 상세 파일 `[작업 결과]` 섹션 작성(커밋 해시 포함) + 상태 🔔 변경
3. `.agent/ACTIVE_TASK.md` 상태 🔄→🔔 변경
4. `gh issue edit 736 --add-label status:review --remove-label status:in-progress`
5. **[문서 커밋]** `[Gemini] docs: TASK-203 완료 보고 — task file 🔔`
6. **[PR 생성]** `feature/teama-task-203-final-freight-settlement-riley → develop`, `Closes #736`
...
| 항목 | 내용 |
|:----|:----|
| 코드 커밋 | `686dab21a9d00b42186a9815a979c4f1a8cb5e52` |
| 회귀 결과 | Vitest unit & regression tests 100% PASS (`rtk npm run test:regression` 검증 완수) |
| 빌드 | 빌드 성공 (`npx tsc --noEmit` 0 error) |
| 특이사항 | SNTL 회의록 W2 P0 요구사항 완수 — 기존 사후청구 및 마감 메커니즘 재사용, 일별/화주별 집계 및 일괄 마감 구축. |
