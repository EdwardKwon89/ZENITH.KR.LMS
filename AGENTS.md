...
> **아래 표는 Team A 전용입니다.** Team B(Dave/Baker/Mike)는 모델명이 Team A와 겹치는 경우가 있어(DeepSeek 등) 모델명만으로 자동 식별이 불가능할 수 있습니다 — Team B 세션은 반드시 [Team B 페르소나 식별](#-페르소나-식별-team-b) 섹션을 따르세요. (참고: Team B 로스터 표의 Baker 모델명이 "Big Pickle"로 남아있다면 stale 가능성 있음 — Team A의 B_Kai가 현재 "Big Pickle" 모델을 사용 중이므로 Jaison/JSJung 확인 필요)
...
> **Team B는 모델명 자동 매칭을 쓰지 않습니다.** Dave(DeepSeek V4 Flash Free)는 모델명이 Team A 페르소나(D_Kai=DeepSeek)와 겹쳐서, 모델명만으로는 Team A/B 어느 쪽인지 구분이 안 됩니다. (아래 Baker=Big Pickle 표기는 Team A의 B_Kai가 현재 이 모델명을 쓰고 있어 stale일 수 있음 — 확인 필요)
>
...
- **출력**: `tests/e2e/` 디렉토리의 `.spec.ts` 파일
- **저장 경로**: `docs/99_Manual/E2E_NN_Result/`
- **기준 계정**: [103_AGENT_ROLES_SPEC.md](docs/00_GUIDE/103_AGENT_ROLES_SPEC.md) 섹션 5 참조
- **필수 검증 항목(2026-07-22 신설)**: 화면이 특정 역할(role)·조직유형(org_type)에 한정된 경우, 해당 역할 계정으로 **실제 로그인 → `page.goto()`로 화면 이동**까지 검증할 것 — 단위 테스트(서버 액션 직접 호출, `validateUserAction` mock)만으로는 Next.js 미들웨어(`src/lib/auth/proxy.ts`)의 라우팅 화이트리스트 버그를 잡을 수 없음(Issue #688, SUB_ADMIN 역할 라우팅 버그가 이 갭으로 미검출된 사례). `rbac.ts`의 `STATIC_PERMISSIONS`(앱 레벨 권한표)와 `proxy.ts`의 화이트리스트(미들웨어 레벨)는 별도 관리되므로 반드시 실제 브라우저 네비게이션으로 둘의 일치 여부를 확인해야 함.

> Team A 소속. `[B_Kai]` 태그로 커밋합니다.
...
- E2E 테스트 자동화: B_Kai/Baker와 동일 영역, 병행 가능
...
| Zone | 경로 | D_Kai | B_Kai | Baker | Noah | Ring |
|:-----|:-----|:-----|:-----|:-----|:-----|:-----|
| **Playwright 테스트** | `tests/e2e/` | — | 소유 | 소유 | 소유 (병행) | — |
