...
> **검수 이력 v3**: E2E-06 PASS 검증 후 실제 구현 기준으로 경로 및 답변 방식 정정 (2026-05-06 Aiden)
...
### E2E-06: VOC 라이프사이클 (등록 → 관리자 답변 → 상태 전환)
- **사전 조건**: 승인된 화주 계정으로 로그인 상태이며, 연결 가능한 오더가 존재.
- **수행 단계**:
    1. 화주가 `/ko/orders/[orderId]` 오더 상세 페이지에서 'VOC' 버튼 클릭 → 유형·제목·내용 입력 후 제출
    2. 어드민 `/ko/voc/admin` 접속 → 신규 VOC 티켓 확인 및 선택
    3. 답변 입력 폼에 직접 텍스트를 입력하여 '답변 등록' 버튼 클릭
- **기대 결과**: 어드민 답변 등록 즉시 VOC 상태가 `OPEN` → `IN_PROGRESS(처리 중)`로 자동 전환되고, 화주가 오더 상세 `Customer Support` 탭에서 상태 변경을 확인 가능.
- **검증 포인트**: `zen_voc` 테이블의 신규 레코드 생성, `zen_voc_answers` 테이블의 `reply_content` 저장, VOC 상태 `IN_PROGRESS` 전환 확인.
- **[수정 이력]** v3 (2026-05-06 Aiden): E2E-06 PASS 검증 후 실제 구현 기준으로 경로 및 답변 방식 정정. VOC(`/voc/admin`)와 QnA(`/support/qna`)는 별개 시스템임을 명확화.
