...
| 화면 URL | /ko/login → /ko/orders |
| 예상 소요 시간 | 20분 |
| 사전 조건 | ADMIN 계정(`admin@zenith.kr`) 로그인 상태, **REGISTERED** 상태 오더 1건 존재, **해당 오더의 경로 선택 완료** (`route_option_id` 존재 — UAT-02-09 선행 수행 또는 오더 상세에서 사전 경로 선택) |
...
| 3 | /ko/orders/{orderId} | 경로 옵션 계산 완료 대기 | — | 전체 운송사 후보 **비교 테이블** 표시 — 직항/경유 그룹 분리, 각 행에 운송사·경로 세그먼트·운송 방식·비용·소요일·추천 배지 표시 | ☐ |
| 4 | /ko/orders/{orderId} | 추천 배지 확인 | — | 최저비용 행 → `최저비용 추천` 배지, 최단시간 행 → `최단시간 추천` 배지, 균형 행 → `균형 추천` 배지 | ☐ |
| 5 | /ko/orders/{orderId} | 비교 테이블 행의 '선택' 버튼 클릭 | — | "경로가 확정되었습니다" 토스트 표시, 해당 행 하이라이트 | ☐ |
...
| 8 | /ko/orders/{orderId} | '경로 재계산' 버튼 클릭 | — | 경로 옵션 재계산, **비교 테이블** 재표시 | ☐ |

### 합격 기준
- [ ] 전 단계 ☑ 완료
- [ ] 오류 메시지 없음
- [ ] 전체 운송사 후보 비교 테이블 표시 (직항/경유 그룹)
- [ ] 각 행에 운송사·경로·비용·소요일 표시, 비용=0인 경우 "요율 미등록" 표시
- [ ] 추천 배지(최저비용·최단시간·균형) 정상 표시
