...
| DEF-030 | UAT-02-03 | 기능 오류 | Y | 수정중 | D_Kai | - | 경로 최적화 구조적 문제 — Order 등록 후 Detail에서 경로 선택하는 역순 설계 + mode 미필터링 + 비용 0 + tiebreaker 부재 | TASK-100 | **① 설계 위상 오류**<br>├ **문제**: 경로 최적화 기능이 Order 등록 단계(운송사·비용 산출) 아닌 Order 상세 페이지에 위치. Order를 먼저 생성한 후 경로를 선택하는 역순 흐름<br>└ **제안**: Order 등록 Multi-step에 Route Step 통합<br><br>**② transport_mode 미필터링**<br>├ **문제** (`routing.ts:34`): `getRouteOptions()`가 `transport_mode`를 RoutingEngine에 전달하지 않음 → DatabaseRouteAdapter가 모든 mode 노출. AIR 등록에도 SEA 표시<br>└ **수정방향**: `IVirtualMapAdapter.getPotentialRoutes()`에 transportMode 파라미터 추가 → DB query에 `.eq('transport_mode', transportMode)` 적용<br><br>**③ 비용 = 0**<br>├ **문제** (`routing.ts:29-31`): `cargo_details={}` → `weight=0` → `chargeableWeight=0` → `baseFreight=0×unit_price=0`<br>└ **수정방향**: `zen_order_packages`로부터 `gross_weight×packing_count + volume` 합산<br><br>**④ 동점 tiebreaker 부재**<br>├ **문제** (`scoring.ts:26`): `selectCostOptimal`이 cost만 비교 — 동점(모두 0) 시 정렬 순서 불확정<br>└ **수정방향**: `a.total_cost - b.total_cost || a.total_transit_days - b.total_transit_days`<br><br>**⑤ RLS (기조치 완료)**<br>├ `20260529150000`: zen_route_network/carriers/rate_cards SELECT에 CORPORATE/INDIVIDUAL 추가<br>└ zen_route_options INSERT/SELECT 정책 신규 생성 | - | Aiden 협의 필요 |
| DEF-031 | UAT-02-01 | 기능 오류 | Y | 수정완료 | D_Kai | - | DEF-022 root cause ① 미완료 수정 — 서버 액션(POST)에서 `zen_last_activity` cookie 갱신 누락 → 서버 액션 성공 후 GET 페이지 이동 시 stale cookie → 로그아웃 | `aae165b` | **배경**: DEF-022 root cause ①에서 server action POST를 idle timeout 체크에서 제외했으나, 조건(`!isServerAction`) 하나로 timeout 체크와 cookie 갱신을 **동시에 회피** → cookie 갱신이 서버 액션에서 전혀 발생하지 않음<br><br>**증상**: Order 등록 폼 작성 후 제출(서버 액션) → `createOrder` 정상 성공 → toast "Created" → 1초 후 `router.push('/orders/{id}')` GET 요청 → middleware: `zen_last_activity`가 stale(30분↑) → signOut → 로그아웃<br><br>**원인** (`proxy.ts:77`): `if (!isAuthPage && purePath !== '/' && !isServerAction)` — 서버 액션에서 cookie 갱신 로직이 아예 실행되지 않음<br><br>**수정**: timeout 체크와 cookie 갱신 로직을 분리<br>├ timeout 체크: 서버 액션 제외 (기존 유지)<br>└ cookie 갱신: 서버 액션 포함 (신규) — 모든 인증 요청에서 `zen_last_activity` 갱신<br><br>**부수효과**: `now = Date.now()`를 블록 상단으로 Hoist — 중복 연산 제거<br><br>**검증**: 회귀 229/229 PASS | - | DEF-022(①) 재오픈 성격 — 동일 commit에서 일괄 수정 |
| DEF-032 | UAT-02-03 | 기능 오류 | Y | 수정완료 | D_Kai | - | TISA Mock → Real Rate Card 연동 미구현 — Order 등록 시 Rate Card 자동 매칭 누락 · Dashboard 실데이터 미표시 · RLS ADMIN 전용 | `6a0dbab` (TASK-104) | **① Rate Card 자동 매칭 로직 누락**<br>├ **문제**: `create_order_atomic` RPC가 Rate Card 탐색 없이 Order만 생성. Order Detail 페이지 TISA snapshot은 Mock 하드코딩(`page.tsx:91-103: rateCardId:'RC-STD-01'`, `baseAmount:1250.00`)<br>├ **필요구현**: Order 등록 시 `origin_port_id`·`dest_port_id`·`transport_mode`(→service_type)·`shipper_id`(→customer_id) 기준으로 `rate_cards` 테이블에서 우선순위 매칭<br>└ 매칭 성공 시 `zen_order_rate_snapshots` INSERT (applied_rule='AUTO')<br><br>**② TISA Dashboard 실데이터 미연동**<br>├ **문제**: `OrderTisaDashboard`가 `snapshot` prop을 받지만, page.tsx에서 Mock 데이터 전달<br>├ **DB 준비**: `zen_order_rate_snapshots` 테이블·제약조건·FK·unique_order_snapshot 이미 존재<br>└ **필요구현**: page.tsx Mock → `getOrderRateSnapshot(orderId)` 서버 액션 호출로 전환<br><br>**③ RLS 정책 부족**<br>├ **문제**: `zen_order_rate_snapshots` SELECT 정책이 ADMIN 전용 — CORPORATE/INDIVIDUAL 접근 불가<br>└ **수정방향**: `zen_orders`와 동일 패턴으로 `is_org_member` 기반 정책 추가<br><br>**④ 설계 확정 (2026-05-31)**<br>├ TISA snapshot 캡처 시점: **경로 선택 완료(route_option_id 설정) 이후**<br>├ 경로 미선택: "No rate snapshot applied yet." fallback 표시<br>└ **IMP-093 · TASK-104 발령** (전제조건: IMP-092·TASK-103 ✅)<br><br>**⑤ 기조치 완료**<br>├ TISA UI 레이아웃·한글 병기·설명: `047b4b0` (DEF-034로 별도 등록)<br>└ Rate Card ID truncate→break-all: `208055e` | ✅ | IMP-093·TASK-104(7225196+6a0dbab) 완료 — Dashboard 실연동·경로선택 시 스냅샷 자동생성·역할별 필드 분기 |
| DEF-033 | UAT-02-03 | 기능 오류 | Y | 수정완료 | D_Kai | - | Order Detail QnA 목록 영원히 빈 배열 + 새문의하기 버튼 invisible | `208055e`·`b97af59` | **① QnA 목록 미표시 (TypeError)**<br>├ **원인** (`OrderQnaSection.tsx:61`): `qnaData.map()` — `getOrderQnaList()`는 `{ qnas: QnaItem[], total }` 반환하는데 객체에 직접 `.map()` 호출 → `TypeError: qnaData.map is not a function`<br>├ catch 블록이 `logger.error()`만 하고 아무 피드백 없이 items 빈 배열 유지 → 사용자는 "조회 안 됨"<br>└ **수정**: `qnaData.qnas.map()` + QnA 작성 후 order detail redirect<br><br>**② 새문의하기 버튼 invisible**<br>├ **원인** (`OrderQnaSection.tsx:168`): `variant="tactile"` → `.zen-tactile`이 `background: #ffffff` 강제 설정. `@layer utilities`에서 Tailwind 유틸리티보다 나중에 선언되어 CSS cascade 우선 → `bg-brand-600` 무시. text도 `text-stone-700`이 `text-white`를 덮음 → 흰 배경 + 회색 글자<br>└ **수정**: `variant="ghost"`로 변경 → `bg-brand-600 text-white` 정상 적용<br><br>**③ QnA 작성 후 flow 개선**<br>├ `createQna`에 `revalidatePath('/orders/{orderId}')` 추가<br>├ `QnaForm` 오더 연계 QnA 작성 시 `/support/qna` 대신 `/orders/{orderId}`로 redirect<br>└ **검증**: 회귀 229/229 PASS | - | - |
| DEF-035 | UAT-02-03 | 기능 오류 | Y | 수정완료 | D_Kai | - | TISA 요율 구조 미분리 — `zen_rate_cards.unit_price` 단일 필드로 운송사 원가·마진·플랫폼 수수료 추적 불가. 화주에게 제시되는 기준 운임의 산출 근거 미검증 | `8132d98` (TASK-103) | **① 구조 문제**<br>├ `zen_rate_cards.unit_price`: All-in Rate인지 carrier cost인지 불명확<br>├ `zen_transport_costs`(unit_cost + profit_margin 15%)와 TISA 매칭 엔진 완전 단절<br>└ Order Detail TISA Snapshot의 Base Amount가 3계층(운송사 원가→운송사 이윤→플랫폼 수수료) 분해 불가<br><br>**② 요구 구조**<br>├ `carrier_cost` (운송사 원가) + `margin_rate`% (운송사 이윤율) + `platform_fee_rate`% (플랫폼 수수료율)<br>└ = `unit_price` (화주 청구 All-in Rate)<br><br>**③ 영향 범위**<br>├ DEF-032(TISA Mock 연동)의 선행 조건 — 실 데이터 연동 전 요금 구조 확립 필요<br>└ `zen_order_rate_snapshots`의 `applied_unit_price` 의미가 불명확해짐 | ✅ | IMP-092·TASK-103(e442ea3+8132d98) 완료 — carrier_cost/margin_rate/platform_fee_rate 3개 컬럼 추가 |
| DEF-034 | UAT-02-03 | 기능 보완 | N | 수정완료 | D_Kai | - | TISA Dashboard UI 개선 — 불필요 부제목 · 영문 단독 · grid 가로배치 · 높이 불일치 | `208055e`·`047b4b0` | **① 부제목 제거**<br>├ **문제**: `Temporal Invariant Snapshot Architecture` — 불필요한 아키텍처 설명<br>└ **수정**: 헤더에 `TISA Rate Snapshot`만 표시<br><br>**② 한글 병기**<br>├ **문제**: 모든 label이 영문 단독 (Rate Card ID, Version, Priority Level, Base Amount)<br>└ **수정**: `Version (버전)`, `Priority (우선순위)`, `Base Amount (기준 운임)`, `Validity Period (유효 기간)`, `Override Reason (재정의 사유)`<br><br>**③ 레이아웃 변경**<br>├ **문제**: `grid-cols-2 lg:grid-cols-4` 가로 배치 — 사이드바에서 정보 밀도 낮음<br>└ **수정**: `flex-col` 세로 배치 + 상단 4개 항목 `grid-cols-2` 카드형(`bg-slate-50 rounded-xl p-3`)<br><br>**④ 높이 매칭**<br>├ **문제**: 좌측 Tracking 패널 대비 TISA 패널 높이가 짧음<br>└ **수정**: `min-h-[320px]` + `flex-1` content 영역<br><br>**⑤ Auto Match 설명 추가**<br>├ **문제**: `Auto Matched` badge만 있고 의미 설명 없음<br>└ **수정**: badge `Auto Matched (자동 매칭)` + 하단 `● Auto Matched: TISA 시스템이 Rate Card를 자동 탐색·매칭하여 스냅샷을 생성했습니다.`<br><br>**⑥ Rate Card ID 전체 표시**<br>├ **문제**: `truncate` class로 ID 잘림<br>└ **수정**: `truncate`→`break-all` + `title` hover tooltip<br><br>**검증**: 회귀 229/229 PASS | - | - |
---
...
| 2026-05-29 | Aiden (Claude) | DEF-030 수정중 전환 — TASK-100 발령(D_Kai). 단기 대안 확정: REGISTERED→상세경로선택→SCHEDULED. ①전체 운송사 비교 ②transport_mode 필터 ③packages 비용 ④tiebreaker ⑤SCHEDULED 가드. IMP-091(Carrier Portal) 별도 등재 |
| 2026-05-30 | D_Kai (OpenCode) | DEF-031 추가 — DEF-022 root cause ① 미완료 수정: server action POST에서 cookie 갱신 누락 → 서버 액션 성공 후 GET 페이지 이동 시 로그아웃 · `aae165b` · 회귀 229/229 PASS · 현황 요약 갱신 (기능오류 수정완료 4→5) |
| 2026-05-30 | D_Kai (OpenCode) | seed 데이터 보완 — `seed-local.ts` `seedOrders()`에 패키지/품목 INSERT 추가 · `verifySeedData()` 자동 무결성 검증 신설 (모든 E2E-SEED 주문의 package·item·weight·dimension 존재 여부 시드마다 자동 체크 → 실패 시 process.exit(1)) · `2efb518`·`c95888d` · 회귀 229/229 PASS |
| 2026-05-30 | D_Kai (OpenCode) | DEF-032·DEF-033·DEF-034 추가 — TISA 실 Rate Card 연동 미구현 / QnA 목록+버튼 버그 / TISA UI 개선 · DEF-024 중복 행 정리 · 현황 요약 갱신 (기능보완 수정완료 1→2, 기능오류 미수정 2→3·수정완료 5→6, 합계 30→33) |
