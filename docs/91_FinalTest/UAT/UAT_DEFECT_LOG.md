...
> ⚠️ **UAT 착수 전 필수 해소**: DEF-032(IMP-093·TASK-104) · DEF-035(IMP-092·TASK-103) — 블로킹 Y, P1
...
| DEF-035 | UAT-02-03 | 기능 오류 | Y | 미수정 | - | - | TISA 요율 구조 미분리 — `zen_rate_cards.unit_price` 단일 필드로 운송사 원가·마진·플랫폼 수수료 추적 불가. 화주에게 제시되는 기준 운임의 산출 근거 미검증 | - | **① 구조 문제**<br>├ `zen_rate_cards.unit_price`: All-in Rate인지 carrier cost인지 불명확<br>├ `zen_transport_costs`(unit_cost + profit_margin 15%)와 TISA 매칭 엔진 완전 단절<br>└ Order Detail TISA Snapshot의 Base Amount가 3계층(운송사 원가→운송사 이윤→플랫폼 수수료) 분해 불가<br><br>**② 요구 구조**<br>├ `carrier_cost` (운송사 원가) + `margin_rate`% (운송사 이윤율) + `platform_fee_rate`% (플랫폼 수수료율)<br>└ = `unit_price` (화주 청구 All-in Rate)<br><br>**③ 영향 범위**<br>├ DEF-032(TISA Mock 연동)의 선행 조건 — 실 데이터 연동 전 요금 구조 확립 필요<br>└ `zen_order_rate_snapshots`의 `applied_unit_price` 의미가 불명확해짐 | - | **IMP-092 · TASK-103 발령** (D_Kai, P1, UAT 전 필수) |
