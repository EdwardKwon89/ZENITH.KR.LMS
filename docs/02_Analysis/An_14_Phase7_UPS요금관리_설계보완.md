...
> **오더 등록 화면 연동(견적 미리보기)은 Team A 범위 밖** — §11 Team B 인계 참조. Team A는 `estimateUpsFreight()` Action을 "요금 조회 API"로 노출만 하고, 오더 등록 화면(`/orders/new`)에 연동하는 것은 Team B(Issue #181) 담당.
...
> 승인 상태: ✅ Edward 승인 완료(2026-07-05) — Team A(Aiden) 구현 담당. 브랜치 `feature/teama-phase71-ups-rate-management` 단일 브랜치로 TASK-171~177 순차 구현 후 통합 PR.
> **2026-07-05 범위 조정(Edward)**: Issue #181(team:b)과의 중복 확인 과정에서 팀 경계 확정 — **Team A = 요금 "등록"(스키마·계산엔진·Admin/Agency 등록 UI), Team B = "Order 등록" 연동 + 정산**. TASK-174는 Action "노출"까지만 Team A 범위이며 오더 등록 화면 연동은 제외(§11).

| Task-ID | 개요 | 선행조건 | 담당 | 산출물 |
|:---|:---|:---|:---:|:---|
| TASK-171 | 스키마: `zen_agency_pricing_policies`·`zen_agency_other_charges` 신규 + `zen_agency_rate_overrides` RLS/트리거 수정 + OC 4종 추가(§3-1·3-2·3-3·3-7) | 본 설계 승인 | Aiden | migration 3~4개, 트리거 단위테스트 — **✅ 완료** |
...
| TASK-174 | `estimateUpsFreight` Action **노출만**(견적 조회 API) — 오더 등록 화면 연동·`agency_org_id` 저장·스냅샷은 **Team B 인계(§11)** | TASK-173 | Aiden | Server Action(계산 API) |
...
| TASK-177 | Ds_11 API 명세 갱신(§6, Team B 인계용 계약 명시) + UAT-17-03 재실행 완료 처리 + 신규 UAT-20/21 작성(§7, 등록 범위만) + 전체 회귀 | TASK-174~176 | Aiden | 명세 갱신, UAT, 회귀 PASS |

**순서**: TASK-171 → TASK-172 → (TASK-173, TASK-175, TASK-176 병행 가능) → TASK-174 → TASK-177

**별도 트랙**: §9 백로그(요율표 구조 정밀화)는 위 표와 무관하게 IMP-146(가칭)으로 분리 발령 — Edward 우선순위 확인 후 별도 Task 채번.

---

## 11. Team B 인계 범위 — Issue #181 (Order 등록 연동 + 정산)

Edward 확정(2026-07-05): **Team A = 요금 "등록"(스키마·계산엔진·등록 UI), Team B = "Order 등록" 연동 + 정산**.

| # | 항목 | 담당 | 비고 |
|:-:|:---|:---:|:---|
| 1 | 오더-Agency 연결 (`zen_orders.agency_org_id` 컬럼 추가 + 오더 생성 시 `zen_agency_shippers` 조회하여 저장) | **Team B** | Issue #181 항목1 |
| 2 | 오더 등록 화면(`/orders/new`) UPS 선택 시 `estimateUpsFreight()`(Team A 제공) 호출해 실시간 견적 표시 | **Team B** | Team A는 Action만 제공, 연동은 Team B |
| 3 | 오더 확정 시 계산 결과 `zen_order_rate_snapshots` 저장(스냅샷) | **Team B** | 기존 TISA 스냅샷 패턴 참고 |
| 4 | 정산 분리 — 화주→Agency, Agency→플랫폼 청구 구분, `/ups/daily-close`·`/settlement` 화면 Agency별 집계 | **Team B** | Issue #181 항목3 |
| 5 | RBAC — AGENCY 화주 역할이 `/orders`, `/ups-rates` 조회 시 본인 소속 Agency 요율만 노출 | **Team B** | 기존 RLS(Team A 구현분) 활용 |

**Team A가 제공하는 API 계약**(Team B가 그대로 소비):
- `estimateUpsFreight(input): UpsFreightEstimate` — Server Action, Zone/제품/무게/Incoterms/대리점ID/화주ID 입력 → 최종 운송비 + breakdown 반환 (§4)
- `fn_get_ups_agency_selling_price(agency_org_id, base_rate_id)` — DB 함수, 화주 세션에서 안전하게 호출 가능
- 상세 계약은 TASK-177에서 `Ds_11_API_상세_명세서.md`에 명문화

**전제조건**: Team B의 #1(오더-Agency 연결)·#2(화면 연동)는 Team A의 TASK-174(Action 노출) 완료 후 착수 가능.

## 12. IMP-146 설계 — 요율표 구조 정밀화 (§9 백로그 착수, 2026-07-05 Edward 발령)

§9에서 식별한 5건 리스크의 기술 설계와 작업 분해. B_Kai·Riley 배정(둘 다 유휴).

### 12-1. 항목별 설계

| # | 항목 | 설계 방향 | 스키마 변경 |
|:-:|:---|:---|:---|
| 1 | 20kg 초과 티어 요금 | 신규 `zen_ups_weight_tier_rates`(product_id, zone_id, tier_min_kg, tier_max_kg NULL 허용, price_per_kg_selling/cost) — 청구중량이 point 테이블 상한(20kg) 초과 시 이 테이블에서 구간 매칭 후 `가격 = 단가 × 청구중량` 계산 | 신규 테이블 |
| 2 | Box 상품 | **스키마 변경 불요** — Box 요금도 실제로는 1kg 단위 point table(1~10, 11~15 등)이라 기존 `zen_ups_base_rates`(0.5kg 배수 CHECK 통과) 재사용 가능. `zen_ups_products`에 `UPS_10KG_BOX`·`UPS_25KG_BOX` 신규 등록 + `max_weight_kg`(10/25) 컬럼 추가해 상한 초과 시 상위 Box로 자동 전환 | `zen_ups_products.max_weight_kg` 컬럼 추가만 |
| 3 | Zone-서비스-방향 매핑 | `zen_ups_zone_countries`에 `product_family`(EXPRESS/SAVER/EXPEDITED/FREIGHT — product_id 대신 상위 그룹으로 단순화)·`direction`(EXPORT/IMPORT) 컬럼 추가, UNIQUE(country_code, product_family, direction)로 복합키 전환. `resolveZoneByCountry()`에 두 파라미터 추가 | 컬럼 2종 추가 + UNIQUE 재정의 (기존 데이터 마이그레이션 필요 — 전 Zone을 EXPRESS/EXPORT로 우선 채움 후 점진 보강) |
| 4 | DWB | 청구중량 기준 금액과 다음 상위 구간(예: 5kg 다음 브레이크포인트) 금액을 모두 계산해 낮은 쪽 채택 — `pricing-engine.ts`에 `applyDeficitWeightBilling()` 추가, 순수 계산 로직만(스키마 변경 없음) | 없음 |
| 5 | Freight 최소운임 | 신규 `zen_ups_freight_minimums`(zone_id, product_id, min_charge_selling, min_charge_cost) — Freight 계열 상품 계산 결과가 최소운임 미만이면 최소운임으로 상향 | 신규 테이블 |

### 12-2. 작업 분해 및 순서

**TASK-179(B_Kai)를 선행**해야 한다 — 항목3(Zone 매핑)이 데이터 모델 기반을 바꾸므로 항목1·4·5보다 먼저 안정화 필요.

| Task | 담당 | 내용 | 선행 |
|:---|:---:|:---|:---|
| TASK-179 | B_Kai | 항목2(Box 상품) + 항목3(Zone-서비스-방향 매핑) — 스키마+마이그레이션+데이터 이관+Admin UI Zone 관리 화면 확장 | 없음 |
| TASK-180 | Riley | 항목1(20kg 초과 티어) + 항목4(DWB) + 항목5(Freight 최소운임) — 스키마 2종 + `pricing-engine.ts` 보강 + 단위테스트 | TASK-179 ✅(Zone 매핑 안정화 후 착수) |

### 12-3. 미결정 사항 (담당 Agent 📝 설계의견 필수)

- 항목3 마이그레이션 시 기존 10-Zone 시드 데이터를 어떤 EXPRESS 계열 매핑으로 초기화할지(현재 시드가 이미 운영 중인 6/30 시범 데이터와 호환되어야 함)
- 항목1 20kg 초과 구간이 실제 시범 운영 중 발생했는지 여부(Edward 확인 필요 — §9 기존 권고 유지)
