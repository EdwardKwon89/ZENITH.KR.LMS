...
### 4.1 사용자 관리 — **격차 낮음, Group(횡단 그룹) 개념만 선택적 도입**
Redmine의 상태 enum(`STATUS_ACTIVE`/`STATUS_LOCKED` 등)·`auth_source` 분기·API 키 지연생성은 ZENITH가 이미 `zen_profiles.status`와 Supabase Auth로 개념적으로 충족하고 있어 **그대로 베낄 필요가 없다**. 유일하게 신규 가치가 있는 것은 **Group**(조직 계층과 무관하게 사용자를 횡단으로 묶는 다대다 관계)이다 — 예: "이번 UAT 참여자만 묶기", "특정 대리점 소속과 무관하게 임시 검수 권한 부여" 같은, 조직(org) 축만으로는 표현 안 되는 임시·횡단 그룹핑이 필요해질 때 검토 가치가 있다. 현재 ZENITH에는 이런 요구가 아직 관측되지 않았으므로 **우선순위는 낮음 — 실제 필요가 생겼을 때 재검토**로 판단한다.

### 4.2 메뉴 관리 — **참조 불필요**
Redmine은 애초에 admin-UI 메뉴 관리 기능이 없다. ZENITH의 `zen_role_permissions` 테이블 기반 접근이 이미 더 발전된 형태다. 이 항목은 Redmine에서 배울 것이 없다.

### 4.3 역할/권한 관리 — **개념은 채택, 저장 방식은 Redmine보다 관계형으로**
...
### 4.4 커스텀필드 관리 — **순수 EAV는 기각, JSONB+필드정의 테이블 하이브리드가 합리적**
...
### 4.5 워크플로우 엔진 — **가장 강하게 채택 권장, tracker=transport_mode 대응이 핵심 근거**
...
| 항목 | 판단 |
|:---|:---|
| 사용자 관리 | 격차 낮음 — Group(횡단 그룹) 개념만 실제 필요 발생 시 재검토 |
