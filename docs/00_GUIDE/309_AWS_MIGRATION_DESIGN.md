...
> **문서번호:** Ds-11 계열 (R-11 API/설계 우선 원칙 적용) | **버전:** v0.6 (초안 — 미승인)
> **작성일:** 2026-08-07 | **작성자:** Aiden (Claude, ZEN_CEO)
> **상태:** ▶️ **재개** (2026-08-14, Edward 지시) — AWS 접속 정보 수령, §8 기준 재개 중. 단 실제 착수 전 추가 확인 필요(§8 참조)
...
- [ ] **SSH 개인키 파일(`.pem`) 실제 내용** — 서버 조사(읽기 전용) 착수의 선행 조건
- [ ] **IAM Access Key/Secret Access Key** — 실제 AWS 리소스(ECS/RDS 등) 구축 작업의 선행 조건(§7.1). 서버 조사는 이것 없이도 SSH만으로 시작 가능
- [ ] 기존 서버(`3.35.197.31`)에 대한 배경 설명 — 어떤 용도로 만들어졌는지, SNTL/JSJung 측 소유인지 등 (§8.2 불확실성 해소)
