...
| v1.7 | 2026-05-14 | Aiden (Claude, ZEN_CEO) | 근본 원인 확정 — §14 추가 |

---

## 14. 근본 원인 확정 (2026-05-14)

> **검증일:** 2026-05-14 | **검증자:** Aiden (Claude, ZEN_CEO)

### 14.1 확정된 근본 원인

**`~/.agents/skills/` 디렉토리 — 1,246개 파일, 72MB**

OpenCode는 세션 시작 시 `~/.agents/skills/`와 `~/.claude/skills/` 두 경로의 모든 스킬 파일을 시스템 프롬프트에 로드한다. 72MB 스킬이 로드되면 컨텍스트 윈도우가 즉시 포화되어 응답 1회마다 컴팩션이 발생하는 구조였다.

로그 증거:
```
06:10:28 → agent=build (응답)
06:10:38 → agent=compaction  ← 10초 만에 컴팩션
06:10:48 → agent=build
06:10:53 → agent=compaction  ← 5초 만에 재컴팩션
```
...
| 이전 진단 | 재판정 |
|:---|:---:|
| GLM Big Pickle 훈련 특성 | ❌ 폐기 |
| AGENTS.md R-02/R-00 충돌 | ⚠️ 부분 유효 (기여 요소) |
| gsd-context-monitor Hook (SAR-2026-05-14-001) | ❌ 메커니즘 오진, 방향(환경)은 유효 |
| **Skill 시스템 과부하 (§3.4)** | **✅ 정확 — 규모만 과소 추정** |

### 14.3 수정 조치 및 결과

```bash
mv ~/.agents ~/.agents.bak  # ~/.agents/skills/ 비활성화
```
...
|:---|:---:|:---:|
| 타 PC (git 동기화) | ❌ 없음 | ✅ 정상 |
| Docker (동일 프로젝트) | ❌ 없음 | ✅ 정상 |
| 이 PC 로컬 | ✅ 1,246개 72MB | ❌ 컴팩션 루프 |
| 이 PC (`~/.agents` 제거 후) | ❌ 없음 | ✅ 정상 |
