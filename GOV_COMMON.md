...
- **실패 관측성(Observability) 의무** (v1.2 신설, 2026-08-22 — TASK-1138 로그 인프라 도입 후 커버리지 조사에서 SHXK/UPS 실패가 로그에 전혀 안 남는 사각지대 다수 확인, DEF-B-142 SHXK 인증키 미등록 사고 미탐지 근본원인과 동일 계열):
  ...
  - **핵심 지점(Critical Point) 실시간 알림 의무** (v1.3 추가, 2026-08-23 — "전수 로깅만으로는 실시간 대응이 안 된다, 소수 핵심 지점은 능동 감시로 강화하자"는 Edward 지시 반영): 로깅만으로는 사람이 대시보드를 들여다봐야 문제를 알 수 있다 — 아래 4개 기준 중 **2개 이상** 해당하는 실패 지점은 로깅에 더해 **실시간 알림(Sentry Alert Rule 또는 Axiom Monitor)**을 반드시 구성한다.
    | 기준 | 질문 |
    |:---|:---|
    ...
    - 즉시성이 중요한 개별 실패(예: 외부 API 호출 실패)는 **Sentry Alert Rule**(이슈 발생 즉시 알림), 누적 패턴이 중요한 실패(예: 배치/정산성 DB 쓰기 실패)는 **Axiom Monitor**(임계치·주기 기반 알림)로 구분 적용한다.
...
> 최신 버전: **v3.4** (2026-08-23) | 전체 이력: [docs/00_GUIDE/GOV_CHANGELOG.md](docs/00_GUIDE/GOV_CHANGELOG.md)
