...
5. **[문서 커밋]** `[Agent] docs: TASK-XXX 완료 보고 — task file 🔔` — task file·ACTIVE_TASK·IMP_PROGRESS 포함
6. ✅ 전환은 Aiden 단독 권한 — Agent 자체 선언 절대 불가

> ⚠️ **커밋 순서 위반 금지**: 코드 커밋 전 task file 🔔 변경 금지. task file 미업데이트 상태로 단일 커밋 금지.

**반복 위반 페널티** (R-17 v1.4 신설):
- 동일 Agent가 **동일 유형 위반(task file 미업데이트·커밋 해시 미기재·상태 미변경)** 누적 **3회 이상** 시:
