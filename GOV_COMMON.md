...
7. **[PR 생성]** `feature/* → develop` PR 생성 — GitHub Issue 번호 연결(`Closes #NNN`) 및 DoD 체크리스트 PR body에 기재. **PR 생성 없이 🔔 완료로 간주하지 않음.**
8. ✅ 전환은 Aiden 단독 권한 (PR 머지) — Agent 자체 선언 절대 불가
...
| v2.1 | 2026-06-17 | Aiden (Claude, ZEN_CEO) | R-17 v2.0 — 완료 보고 7단계 신설: PR 생성(`feature/* → develop`) 필수화. `Closes #NNN` GitHub Issue 연결 의무. PR 미생성 시 🔔 미완성으로 간주. GitHub Issues/PR 전환 체계 적용 (신규 Task TASK-151~부터 GitHub Issue 기반 발령). `.github/` Foundation 구축 (CODEOWNERS·PR Template·Issue Template·GitHub Actions). |
| v2.2 | 2026-06-18 | Aiden (Claude, ZEN_CEO) | R-17 v2.1 — 착수 절차 §0 신설: Git 동기화 필수화(fetch→develop pull→본인 브랜치 checkout→rebase). 공유 workspace 교차 오염 방지. pre-commit hook Agent 태그↔브랜치 소유주 불일치 차단 추가. IMP-123 등재 (Issue #31 기반, Edward 승인). |
