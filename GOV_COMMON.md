...
### R-16 | 세션 시작 시 상태 파일 일관성 검증 의무
세션 최초 시작 시 `ACTIVE_AGENT.md`의 Status가 BUSY인 경우, `TASK_BOARD.md` 활성 태스크 현황과 교차 검증하여 해당 작업이 실제로 진행 중인지 확인한다. 불일치 발견(BUSY이지만 TASK_BOARD에 해당 작업 없음) 시 착수 전 Aiden에게 보고하고 정정 지시를 기다린다. (SAR-2026-05-13-001)
