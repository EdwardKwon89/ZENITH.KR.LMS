...
0. **[Git 동기화 — 필수, 예외 없음]** 작업 시작 전 반드시 아래 순서를 실행한다:
   ...
   **[Git worktree 격리 — 필수, 2026-07-11 강화]** 위 순서만으로는 부족하다 — 여러 에이전트가 **동일한 물리 디렉토리**를 공유하면 브랜치를 정확히 갈아탔더라도 한 에이전트의 미커밋 변경분이 다른 에이전트의 다음 커밋에 섞여 들어갈 수 있다(Issue #358 — Mike의 첫 PR#353에 Dave의 로컬 Issue #294 미반영 커밋이 혼입된 사례). 반드시 에이전트마다 **독립된 `git worktree`**에서 작업한다:
   ```bash
   git worktree add -b feature/[본인-브랜치] /tmp/wt-[본인이름]-[작업명] origin/develop
   cd /tmp/wt-[본인이름]-[작업명]
   # 작업·커밋·push 완료 후
   git worktree remove /tmp/wt-[본인이름]-[작업명] --force
   ```
   같은 공유 디렉토리에서 `git checkout`만으로 브랜치를 전환하며 작업하지 않는다.
   > **근거**: 공유 물리 디렉토리 사용 시 브랜치 전환 규율을 지켜도 교차 오염 가능. (Issue #358, 2026-07-11)
