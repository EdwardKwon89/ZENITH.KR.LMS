...
전제조건 미충족(🚫) 태스크는 착수하지 않는다. 상태 불일치(ACTIVE_TASK ↔ 상세 파일) 발견 시 착수 전 Aiden에게 보고하고 정정 지시를 기다린다. (SAR-2026-05-13-001 반영 및 신규 오케스트레이션 체계 확장)
...
3. 상세 파일 생성 (`.agent/tasks/TASK-{ID}_{YYMMDD}_{개요}_{Agent}.md`)
4. 상세 파일 상태 ⬜→🔄 + ACTIVE_TASK.md 상태 동시 반영

**완료 보고 절차**:
1. 상세 파일 [작업 결과] 섹션 작성
2. 상세 파일 상태 🔄→🔔 + ACTIVE_TASK.md 상태 동시 반영
3. ✅ 전환은 Aiden 단독 권한 — Agent 자체 선언 절대 불가

**파일 조작 규칙**:
- 상세 파일은 담당 Agent만 수정 가능
- ACTIVE_TASK.md는 상태 반영 목적으로만 수정 (내용 추가 금지)
- 전제조건 블로커(🚫→⬜) 해제는 Aiden 전속
