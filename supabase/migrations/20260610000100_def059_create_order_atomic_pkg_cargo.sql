-- DEF-059: create_order_atomic RPC에서 special_cargo_type PKG 레벨로 이전
-- 1. zen_orders INSERT에서 special_cargo_type 제거 (DEPRECATED, 컬럼 유지)
-- 2. packages jsonb_to_recordset에 special_cargo_type 추가
-- 3. zen_order_packages INSERT에 special_cargo_type 포함
...
BEGIN
  ...
  -- 2. zen_orders Header 삽입 (special_cargo_type 제거 — PKG 레벨로 이전)
  ...
  -- 4. Packages 및 Items 삽입 (special_cargo_type PKG 레벨)
