...
CREATE TABLE common_code_groups (
    group_code TEXT PRIMARY KEY, -- 그룹 식별자 (e.g., 'CARGO_STATUS')
    group_name TEXT NOT NULL, -- 명칭
    description TEXT,
    is_system BOOLEAN DEFAULT false, -- 시스템 예약 여부
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now()
);
-- 2. 공통 코드 상세 테이블 (소분류)
CREATE TABLE common_codes (
    group_code TEXT REFERENCES common_code_groups(group_code) ON DELETE CASCADE,
    code_value TEXT NOT NULL, -- 실제 코드 값 (e.g., '10', 'AIR')
    code_name_ko TEXT NOT NULL, -- 명칭 (e.g., '접수완료', '항공')
    code_name_en TEXT,
    description TEXT,
...
CREATE POLICY "Common codes are viewable by all authenticated users" ON common_codes
    FOR SELECT USING (auth.role() = 'authenticated');
-- 오직 ADMIN만 코드 정보를 수정할 수 있음 (zen_profiles 테이블의 role 필드 참조)
