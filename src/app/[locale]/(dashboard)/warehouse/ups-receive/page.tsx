...
import UpsReceiveProcessForm from '@/components/warehouse/UpsReceiveProcessForm';

export default async function WarehouseUpsReceivePage({
  params,
}: {
  ...
  const t = await getTranslations('WarehouseUpsReceiving');
  ...
  return (
    <div className="relative min-h-screen animate-in fade-in duration-500 bg-slate-50/50 pb-12">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest opacity-70 ml-3.5">
              {navT('logistics_group')} — {navT('logistics_ups_receiving')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8">
        <UpsReceiveProcessForm locale={locale} />
      </div>
    </div>
  );
}
