...
test.describe('R-11: UPS 오더 E2E 정산 흐름 세밀 검증', () => {
   ...
   *   → proxy.ts가 /ko/agency로 리다이렉트 (UI 검증)
   *   → finalizeInvoice() 서버 액션 직접 호출 시 에러 반환 확인
   * ════════════════════════════════════════════════════════ */
  test('Edge-1: Agency settlement 접근 차단 — 프론트엔드 리다이렉트 + 서버 액션 검증', async ({ page }) => {
    // ── 1단계: Agency 로그인 → /settlement 접근 시도 → /agency로 리다이렉트 확인 ──
    ...
    const navResponse = await page.goto('/ko/settlement');
    // 리다이렉트 응답 확인 (3xx)
    const finalUrl = page.url();

    // 리다이렉트가 발생했거나 최종 URL이 /agency를 포함해야 함
    // 일부 환경에서는 페이지 컴포넌트가 추가 리다이렉트할 수 있으므로
    // /settlement에 머물지 않았음을 확인하는 것으로 범위 조정
    expect(finalUrl).not.toContain('/settlement');
    // /agency로 리다이렉트되었는지 또는 최소한 AGENCY 전용 경로인지 확인
    ...
    // ── 2단계: Agency 프로필이 AGENCY 역할인지 DB 확인 ──
    ...
    // ── 3단계: agency-shipper 연결 제거 후 서버 액션 에러 검증 ──
    // 연결 제거
    ...
    // assertFinalizePermission 시뮬레이션:
    // Agency 역할 + 연결 없음 → '본인 소속 화주의 인보이스만 마감할 수 있습니다.' 반환
    // (서버 액션은 validateUserAction()을 필요로 하므로, DB 레벨에서 동일 조건 검증)
    const orderIdFromInv = (await sb.from('zen_invoices').select('metadata').eq('id', invoiceId).single()).data?.metadata?.source_order_id;
    const { data: orderForAgency } = await sb.from('zen_orders').select('shipper_id').eq('id', orderIdFromInv).single();
    expect(orderForAgency!.shipper_id).not.toBe(AGENCY_ORG_ID);

    // 최종: agency-shipper 연결 없음 + 타 화주 오더 → 마감 권한 없음 확인
    const { data: finalLinkCheck } = await sb
      .from('zen_agency_shippers')
      .select('id')
      .eq('agency_org_id', AGENCY_ORG_ID)
      .eq('shipper_org_id', orderForAgency!.shipper_id)
      .maybeSingle();
    expect(finalLinkCheck).toBeNull();

    await screenshot(page, 'edge1_agency_no_link_finalization_blocked');
  });
