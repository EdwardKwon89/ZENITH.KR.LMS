...
describe('TC-P6-DB-04: 통합 서비스 요율 조회 엣지 케이스 통합 테스트', () => {
  ...
  it('운송 요율 last tier fallback (weight > max_weight) — 마지막 tier rate * weight 적용', async () => {
    ...
    supabase._listResult = {
      data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: [{ min_weight: 0, max_weight: 100, rate: 5 }], currency: 'USD', carrier: { name: 'Test Air' } }],
      error: null,
    };
  ...
  it('carrier name null — 빈 문자열 fallback', async () => {
    ...
    supabase._maybeSingleResult = { data: { id: 'port-icn-uuid' }, error: null };
    supabase._listResult = {
      data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: [{ min_weight: 0, max_weight: 1000, rate: 3 }], currency: 'USD', carrier: null }],
      error: null,
    };
    (validateUserAction as any).mockResolvedValue({ supabase, profile: { id: 'user-1', role: USER_ROLES.CORPORATE, org_id: 'org-1' } });
  ...
  it('운송 요율만 존재하고 customs/delivery 빈 배열 — 부분 결과 성공', async () => {
    ...
    supabase._listResultQueue = [
      Promise.resolve({ data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: [{ min_weight: 0, max_weight: 100, rate: 5 }], currency: 'USD', carrier: { name: 'Korean Air' } }], error: null }),
