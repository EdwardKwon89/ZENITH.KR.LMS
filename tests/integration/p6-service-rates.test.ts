...
describe('TC-P6-DB-04: 통합 서비스 요율 조회 엣지 케이스 통합 테스트', () => {
  ...
  it('운송 요율 last tier fallback (weight > max_weight) — 마지막 tier rate * weight 적용', async () => {
    ...
    supabase._listResult = {
      data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: { weight_slabs: [{ weight_min: 0, unit_price: 5 }], cbm_slabs: [{ cbm_min: 0, cbm_price: 0, min_charge: 0 }] }, currency: 'USD', carrier: { name: 'Test Air' } }],
      error: null,
    };
  ...
  it('운송 요율 tiers 빈 배열 — estimatedCost 0 반환', async () => {
    ...
    supabase._maybeSingleResult = { data: { id: 'port-icn-uuid' }, error: null };
    supabase._listResult = {
      data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: { weight_slabs: [], cbm_slabs: [] }, currency: 'USD', carrier: { name: 'Test Air' } }],
      error: null,
    };
    (validateUserAction as any).mockResolvedValue({ supabase, profile: { id: 'user-1', role: USER_ROLES.CORPORATE, org_id: 'org-1' } });
  ...
  it('carrier name null — 빈 문자열 fallback', async () => {
    ...
    supabase._maybeSingleResult = { data: { id: 'port-icn-uuid' }, error: null };
    supabase._listResult = {
      data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: { weight_slabs: [{ weight_min: 0, unit_price: 3 }], cbm_slabs: [{ cbm_min: 0, cbm_price: 0, min_charge: 0 }] }, currency: 'USD', carrier: null }],
      error: null,
    };
    (validateUserAction as any).mockResolvedValue({ supabase, profile: { id: 'user-1', role: USER_ROLES.CORPORATE, org_id: 'org-1' } });
  ...
  it('운송 요율만 존재하고 customs/delivery 빈 배열 — 부분 결과 성공', async () => {
    ...
    supabase._listResultQueue = [
      Promise.resolve({ data: [{ id: 'rate-1', carrier_id: 'carrier-1', transport_mode: 'AIR', tiers: { weight_slabs: [{ weight_min: 0, unit_price: 5 }], cbm_slabs: [{ cbm_min: 0, cbm_price: 0, min_charge: 0 }] }, currency: 'USD', carrier: { name: 'Korean Air' } }], error: null }),
