...
describe('TASK-B-188: DEF-117 RLS AGENCY 검증', () => {
  ...
  describe('마이그레이션 구조 검증', () => {
    ...
    it('CI 환경 GRANT 문 포함 (authenticated 롤)', () => {
      ...
      expect(migration).toContain('GRANT INSERT ON public.zen_ups_labels TO authenticated');
      expect(migration).toContain('GRANT UPDATE ON public.zen_ups_labels TO authenticated');
      expect(migration).toContain('GRANT INSERT ON public.zen_ups_label_errors TO authenticated');
    });
  });
