...
| 2026-07-05 | Aiden (Claude, ZEN_CEO) | **IMP-146 TASK-180 설계 확정(📝→🔍)** — Riley 설계의견을 공식 UPS Rate Guide 원문(p.2·p.17/18)과 대조 검증, DWB·20kg 경계 반올림 규칙 모두 정확 확인해 승인. 검증 중 **DEF-095**(WW_EXPEDITED 상품에 0.5kg 반올림 오적용 — 정답 1kg, 기 병합된 Phase 7.1 코드의 결함, High) 신규 발견 — R-18에 따라 Edward 보고 후 우선순위 결정 대기. TASK-180 DoD에 반올림함수 교체+DEF-095 해소 2건 추가. TASK-179(B_Kai)는 설계의견 미제출로 검토 보류. |
| 2026-07-05 | Aiden (Claude, ZEN_CEO) | **IMP-146 TASK-179 설계 확정(📝→🔍)** — B_Kai 설계의견(Zone 매핑 Fallback 2단계 체인 + `resolveZoneByCountry()` 호출부 영향 분석) 직접 코드 확인으로 재검증 후 승인. DoD에 `fallbackApplied` 투명성 필드 1건 추가. 즉시 착수 가능(전제조건 없음). |
| 2026-07-05 | B_Kai (Big Pickle) | **IMP-146 🔔 TASK-179 (SPR-01) 완료 코드 커밋 완료** — Migration 2종(Box 상품·Zone 매핑), Types 확장, Server Actions/UPS Engine 보강(`resolveZoneByCountry` Fallback 2단계 + `fallbackApplied`), Admin UI(Box max_weight_kg 필드 + Zone product_family/direction 셀렉터). TC-UPS-ZONEMAP-01/02/03 신규 7케이스. 회귀 436/436 PASS. tsc 신규 오류 0건. PR #190 생성(Aiden 검토 대기). |
| 2026-07-05 | Aiden (Claude, ZEN_CEO) | **TASK-181 Hotfix ✅ 완료 — DEF-095 해소** — Edward 지시("DEF-095 Hotfix로 먼저 처리")로 TASK-180 완료 대기 없이 즉시 처리. `resolveBillingWeight()` 신규 함수로 `ceilToHalfKg()` 전체 호출부 교체(WW_EXPEDITED 상시 1kg / 그 외 20kg 이하 0.5kg·초과 1kg). TC-UPS-EXPEDITED-ROUND-01~05 신규. 코드 커밋 `b1d0725`. TASK-180 DoD 중 반올림함수 교체 항목 선반영 완료 — Riley 공유 필요. |
...
| 2026-07-05 | Riley (Gemini) | **IMP-146 🔔 TASK-180 (SPR-02) 완료 코드 커밋 완료** — Migration 1종(20kg 초과 per-kg 요율 및 Freight 최소운임 스키마 + Z2~Z10 대표 6개 제품/10구간 seed 적재), Types 확장, `computeUpsFreight` 및 `buildBreakdown` calculation engine 구현(20kg 초과 분기 + DWB 비교 + Freight 최소운임 capping 및 breakdown 투명성 제공), `estimateUpsFreight` server action 연동(tier rates 및 freight min 조회 추가). TC-UPS-TIER/DWB/FREIGHTMIN-* 신규 7케이스. 회귀 443/443 PASS. tsc 신규 오류 0건. PR #196 생성 예정. |
