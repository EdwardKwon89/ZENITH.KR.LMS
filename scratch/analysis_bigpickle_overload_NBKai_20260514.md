...
### 2.3 진짜 원인: `~/.agents/skills/` Skill 파일 1,246개 (72MB)

**문제 PC의 `~/.agents/skills/` 디렉토리에 1,246개 Skill 파일이 72MB 용량으로 존재.**

| 요소 | 문제 PC | 정상 PC |
|:----|:-------:|:-------:|
| `~/.agents/skills/` 파일 수 | **1,246개 (72MB)** | 없음 또는 소수 |
| rtk / gsd 설치 | ✅ | ❌ |
| 무한 루프 발생 | ✅ | ❌ |
...
ECC(Everything Claude Code)가 마켓플레이스의 모든 Skill을 `~/.agents/skills/`에 일괄 다운로드하면서 발생하는 문제. Skill 하나하나가 시스템 프롬프트에 "available skills"로 포함되어, 모델이 매 입력마다 전체 Skill 목록을 스캔하려고 시도함.
...
**문제 PC 실제 경로:** `~/.agents/skills/` (홈 디렉토리 절대 경로)

```bash
# 1단계: 백업 (2026-05-14 Edward 완료)
# mv ~/.agents ~/.agents.bak

# 2단계: ~/.claude/skills/ broken symlink 제거
find ~/.claude/skills -maxdepth 1 -type l -delete

# 3단계: 필요 Skills만 ~/.agents/skills/에 복원
mkdir -p ~/.agents/skills
# §3.2 목록의 12개 Skill만 선별하여 복사
```

> ⚠️ 상대 경로(`.agent/skills/`)가 아닌 **절대 경로(`~/.agents/skills/`)** 가 실제 Skills 위치. ECC 설치 시 `~/.agents/skills/`에 실제 파일을 저장하고 `~/.claude/skills/`에 symlink를 생성하는 구조.
...
2. **정기 정리**: `~/.agents/skills/`를 프로젝트 실제 필요 기술만 유지하도록 관리
3. **`~/.gitignore_global`에 `.agents/skills/` 추가**하여 다른 PC로 Skill 과다 문제가 전파되지 않도록 방지
...
| **`~/.agents/skills/` Skill 1,246개 (72MB) 과다** | **✅ 진짜 원인** |
