...
- **개선 사항 도출 및 기록 의무 (Improvement Logging, R-15)**: 작업 수행 중 즉시 수정이 불가하거나 별도 계획이 필요한 개선 사항(기능 결함, 구조적 한계, 기술 부채 등)을 발견한 경우, 해당 내용을 **반드시** `scratch/post_launch_improvements.md` 파일에 즉시 기록해야 합니다.
  - **파일 경로**: `scratch/post_launch_improvements.md`
  - **항목 형식**: `IMP-NNN` (자동 증가 일련번호)
  - **필수 기재 항목**: 발견 경위, 현재 상태, 임시 조치 내용(있는 경우), 목표 구현, 관련 파일, 예상 공수, 우선순위
  - **기록 시점**: 임시 픽스 적용 직후 또는 이슈 발견 즉시 (완료 보고 전 반드시 기록)
  - **우선순위 기준**: `Critical` (서비스 중단 위험) / `High` (요구사항 미충족) / `Medium` (UX 저하) / `Low` (기술 부채)
...
| v1.14| 2026-05-08 | Claude (Aiden) | 개선 사항 도출 및 기록 의무 R-15 추가 (post_launch_improvements.md 관리 규칙) |
